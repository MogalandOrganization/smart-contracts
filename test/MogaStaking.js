...
describe("MogaStaking contract", function () {
  ...
  let yearMaturity = new BN(365 * 86400); // 1 year
  ...
  this.beforeEach(async function () {
    ...
    [deployerAcc, mogaAdmin, addr1, addr2, addr3] =
    ...
    await mogaToken
      ...
      .transfer(
        mogaStakingAddress,
        hre.ethers.parseEther(initialStakingAmount)
      );
    const stakingBalance = await mogaToken.balanceOf(mogaStakingAddress);
    //console.log(stakingBalance);
    expect(Number(hre.ethers.formatEther(stakingBalance))).to.equal(
      Number(initialStakingAmount)
    );
    ...
    await mogaStaking
      .connect(mogaAdmin)
      .setNewFlexibleRewardRate(hre.ethers.parseEther(rate.toString()));
    ...
    await mogaToken
      .connect(mogaAdmin)
      .transfer(addr1.address, hre.ethers.parseEther(stakeAmount));
    ...
    await mogaToken
      .connect(mogaAdmin)
      .approve(mogaStakingAddress, hre.ethers.parseEther(initialStakingAmount));

    const addr1Balance = await mogaToken.balanceOf(addr1.address);
    expect(Number(hre.ethers.formatEther(addr1Balance))).to.equal(
      Number(stakeAmount)
    );
    ...
    await mogaToken
      .connect(addr1)
      .approve(mogaStakingAddress, hre.ethers.parseEther(stakeAmount));

    // user addr2 and addr3 prep
    // we do not approve moga staking contract to test fail scenarios
    await mogaToken
      .connect(mogaAdmin)
      .transfer(addr2.address, hre.ethers.parseEther(stakeAmount));
    const addr2Balance = await mogaToken.balanceOf(addr2.address);
    expect(Number(hre.ethers.formatEther(addr2Balance))).to.equal(
      Number(stakeAmount)
    );
  ...
  describe("Deployment", function () {
    ...
    it("should have " + initialStakingAmount + " rewards", async function () {
      let totalRewards = Number(
        hre.ethers.formatEther(await mogaStaking.totalRewards())
      );
  ...
  describe("interest rate calculations", function () {
    it("Check yearlyRateToRay() function", async () => {
      ...
      tinyRateRay = await mogaStaking.yearlyRateToRay(
        hre.ethers.parseEther(tinyRate.toString())
      );
      // console.log(tinyRateRay);
      // console.log(yearlyRateToRay(tinyRate));
      expect(tinyRateRay).to.eq(yearlyRateToRay(tinyRate).toFixed(0));

      smallRateRay = await mogaStaking.yearlyRateToRay(
        hre.ethers.parseEther(smallRate.toString())
      );
      expect(smallRateRay).to.eq(yearlyRateToRay(smallRate).toFixed(0));

      rateRay = await mogaStaking.yearlyRateToRay(
        hre.ethers.parseEther(rate.toString())
      );
      expect(rateRay).to.eq(yearlyRateToRay(rate).toFixed(0));

      bigRateRay = await mogaStaking.yearlyRateToRay(
        hre.ethers.parseEther(bigRate.toString())
      );
      expect(bigRateRay).to.eq(yearlyRateToRay(bigRate).toFixed(0));

      hugeRateRay = await mogaStaking.yearlyRateToRay(
        hre.ethers.parseEther(hugeRate.toString())
      );
    ...
    it("Check accrueInterest() function", async () => {
      ...
      for (let i = 0; i < testCases.length; i++) {
        ...
        newPrincipal = await mogaStaking.accrueInterest(
          _principal.toString(),
          _rateRay.toString(),
          _age.toString()
        );
  ...
  describe("fixedTerm staking transactions", function () {
    ...
    it("admin: Should create stakeFixedTermForBeneficiary", async function () {
      ...
      expect(
        await mogaStaking
          .connect(mogaAdmin)
          .stakeFixedTermForBeneficiary(
            1,
            hre.ethers.parseEther(stakeAmount),
            addr2.address
          )
      ).to.emit(mogaStaking, "StakeOfferCreated");
    });
    it("staking should create correct stake struct", async function () {
      ...
      await expect(
        mogaStaking
          .connect(addr1)
          .stakeFixedTerm(1, hre.ethers.parseEther(stakeAmount))
      ).to.changeTokenBalances(
        mogaToken,
        [addr1, mogaStaking],
        [
          hre.ethers.parseEther(stakeAmount) * -1n,
          hre.ethers.parseEther(stakeAmount),
        ]
      );
      let [stakeOfferId, principle, created, owner] =
        await mogaStaking.getStakeDetails(1);
      ...
      expect(await mogaStaking.totalStaked()).to.eq(
        hre.ethers.parseEther(stakeAmount)
      );
    });
    ...
    it("beneficiaries can unstake after lockup period", async function () {
      ...
      expect(await mogaStaking.connect(addr2).unStakeFixedTerm(1)).to.emit(
        mogaStaking,
        "Withdraw"
      );
    });
    it("should increase totalInterest by to be commited rewards", async function () {
      await mogaStaking
        .connect(mogaAdmin)
        .createStakeOffer(
          hre.ethers.parseEther(rate.toString()),
    ...
    describe("validations", function () {
      it("should revert if staking offer does not exist", async function () {
        await expect(mogaStaking.connect(addr2).stakeFixedTerm(99, stakeAmount))
          .to.be.reverted;
      });
      it("should revert if staking offer is only accessible by admin", async function () {
        ...
        await expect(mogaStaking.connect(addr2).stakeFixedTerm(2, stakeAmount))
          .to.be.reverted;
      ...
      it("should revert if unstaking is attempted by other than stake Owner", async function () {
        ...
        await expect(mogaStaking.connect(addr2).unStakeFixedTerm(1)).to.be
          .reverted;
      });
      it("should revert if unStakeFixedTerm is attempted prior to lockup period", async function () {
        ...
        await expect(mogaStaking.connect(addr1).unStakeFixedTerm(1)).to.be
          .reverted;
      });
      ...
      it("should revert if staking address not approved", async function () {
        ...
        await expect(mogaStaking.connect(addr2).stakeFixedTerm(1, stakeAmount))
          .to.be.reverted;
      });
      ...
      it("should revert if address has insufficient balance", async function () {
        ...
        await expect(mogaStaking.connect(addr2).stakeFixedTerm(1, totalSupply))
          .to.be.reverted;
      });
    });
    ...
    describe("events", function () {
      ...
      it("should emit Withdraw event", async function () {
        ...
        await expect(mogaStaking.connect(addr1).unStakeFixedTerm(1)).to.emit(
          mogaStaking,
          "Withdraw"
        );
      });
    ...
    describe("flexible stake transactions", function () {
      it("should deposit amount in flexible stake", async function () {
        await expect(
          mogaStaking
            .connect(addr1)
            .stakeFlexible(hre.ethers.parseEther(stakeAmount))
        ).to.changeTokenBalances(
          mogaToken,
          [addr1, mogaStaking],
          [
      ...
      it("Should have a flexibleBalanceOf(address) equal to amount deposited", async function () {
        await mogaStaking
          .connect(addr1)
          .stakeFlexible(hre.ethers.parseEther(stakeAmount));
        expect(await mogaStaking.flexibleBalanceOf(addr1.address)).to.eq(
          hre.ethers.parseEther(stakeAmount)
        );
      });
      it("Should have a flexibleLastUpdated(address) equal to the latest block timestamp", async function () {
        ...
        expect(await mogaStaking.flexibleLastUpdated(addr1.address)).to.eq(
          latest
        );
      });
      it("Should not change the total reward balance", async function () {
        ...
        expect(Number(hre.ethers.formatEther(rewardBalance))).to.equal(
          Number(initialStakingAmount)
        );
      });
      it("Should not change the total interest", async function () {
        // flexible staking total interest cannot be pre-calculated as flexible stajing runs forever so it will stay zero
        await mogaStaking
          .connect(addr1)
          .stakeFlexible(hre.ethers.parseEther(stakeAmount));
      ...
      describe("validations", function () {
        it("should revert if staking address not approved", async function () {
          await expect(mogaStaking.connect(addr2).stakeFlexible(stakeAmount)).to
            .be.reverted;
        });
        ...
        it("should revert if address has insufficient balance", async function () {
          ...
          await mogaToken
            .connect(addr2)
            .approve(mogaStakingAddress, totalSupply);
          await expect(mogaStaking.connect(addr2).stakeFlexible(totalSupply)).to
            .be.reverted;
        });
      });
      describe("events", function () {
    ...
    describe("flexible Compound", function () {
      ...
      describe("Events", function () {
        it("Should emit Compound event", async function () {
          await expect(
            mogaStaking.connect(addr1).compoundFlexible(addr1.address)
          )
    ...
    describe("Withdraw flexible stake", async function () {
      ...
      it("Should change token balances", async function () {
        let stakePlusInterestAfterFees = hre.ethers.parseEther(
          "104.896389859108291667"
        );

        let withdrawBalance = hre.ethers.parseEther("104.665669918103188919");

        await expect(
          mogaStaking.connect(addr1).withdrawFlexible()
        ).to.changeTokenBalances(
          mogaToken,
          [addr1, mogaStaking],
      ...
      describe("Validations", function () {
        ...
        it("Should revert if flexible stake does not exist", async function () {
          await expect(
            mogaStaking.connect(addr2).withdrawFlexible()
          ).to.be.revertedWithCustomError(
            mogaStaking,
            "FlexibleStakeBalanceIsZero"
          );
        ...
        it("Should reduce MOGA circulating supply after burning during flexible unstake", async function () {
          ...
          expect(
            Number(hre.ethers.formatEther(newSupply)) <
              Number(hre.ethers.formatEther(currentSupply))
          );
        });
      });
    });
