...
describe("MogaToken contract", function () {
  ...
  describe("Transactions", function () {
    it("Should transfer tokens between accounts", async function () {
      ...
      await mogaToken
        .connect(mogaAdmin)
        .transfer(addr1.address, hre.ethers.parseEther("50"));
      ...
      await mogaToken
        .connect(addr1)
        .transfer(addr2.address, hre.ethers.parseEther("50"));
    ...
    it("Should fail if sender doesn't have enough tokens", async function () {
      ...
      await expect(
        mogaToken.connect(addr1).transfer(mogaAdmin.address, 1)
      ).to.be.revertedWithCustomError(mogaToken, "ERC20InsufficientBalance");

      // mogaAdmin balance shouldn't have changed.
      expect(await mogaToken.balanceOf(mogaAdmin.address)).to.equal(
        initialOwnerBalance
      );
    });
    ...
    it("Should update balances after transfers", async function () {
      ...
      await mogaToken
        .connect(mogaAdmin)
        .transfer(addr1.address, hre.ethers.parseEther("100"));
      ...
      expect(Number(hre.ethers.formatEther(finalOwnerBalance))).to.equal(
        999999850
      );
